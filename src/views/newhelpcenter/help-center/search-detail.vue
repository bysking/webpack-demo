<template>
    <div class='container' v-fixStatusBar>
      <Title @on-back="backToIndex" :customBack="true" value='帮助详情'></Title>
      <!-- 标题 -->
      ...
      <div class="scroll-wrap border-bottom-1px" ref="refWrap">
        <yd-scroll
          v-fixTabBar
          ref="scrolldetail"
          class="scroll"
        ...
        >
          <ul class="y-list" v-if="relateTips.length >= 0">
            ...
            <div class="no-data" v-show="!relateTips || relateTips.length === 0">暂无数据</div>
          </ul>
        </yd-scroll>
      </div>
      <div class="bottom-btn" ref="refOfBtn">
        <div>
          <div @click="toTipPage">
            <i class="active cubeic-back"></i>
          </div>
        </div>
        <div>
          <div>
